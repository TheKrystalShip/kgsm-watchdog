...
the watchdog's open failed, every sweep found the router unreachable and correctly did nothing, and
nothing anywhere said so. Servers sat unreachable from the internet until somebody read the router's
own table and toggled its UPnP setting.

- **`upnp-router` is a degraded component.** Every `upnpc` call — a start's open, a stop's close, a
  sweep, a `/upnp` query — reports whether a router answered to `UpnpRouterHealth`. Calls unanswered
  across 15 minutes (or two sweeps, when the sweep is slower) with no gap wide enough to hide an answer
  write `leaf.degraded` with a sentence saying forwards cannot be opened or restored; the first answer
  writes `leaf.recovered`. While it stands, the sweep keeps listing even with nothing forwarding, so
  the recovery is measured.
- **A call whose discovery gets no answer is retried at the router's last known address.** The
  description URL is lifted off each successful discovery, persisted as `upnp-gateway` in the state
  directory, and handed to `upnpc -u`, which skips discovery. A router whose discovery service has
  died while its control service lives keeps opening forwards; a warning is logged when that starts
  and a line when discovery answers again.
